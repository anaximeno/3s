...
struct binarytree {
     ...
     * */
    int (*search) (b_tree* self, value_t value);

    /* Searchs for a value, returning the level it was first found, after
     * the given level.
     * */
    int (*search_after_level) (b_tree* self, value_t value, unsigned level);
     ...
     * */
    void (*remove) (b_tree* self, value_t value);

    /* Removes the given value if it is in the given level of the tree. */
    void (*remove_at_level) (b_tree* self, value_t value, unsigned level);
